...
def random_line(N, max_value , length_scale):
       ...
       ffn = f_fn(x)
       ffn_norm = (max_value*(ffn - np.min(ffn))/np.ptp(ffn))
...
# BC_left = random_line( N = nrow , max_value = 10 , length_scale = 0.5)
...
duration = pd.to_timedelta(["0", "200d"])
# Collect time discretization
times = start_date + duration

wells_active = True # wells for boundary conditions
# Well
well_layer = np.ones(shape=(nrow-1)).astype('int')
well_row = np.arange(1,nrow).astype('int')
well_column = np.ones(shape=(nrow-1)).astype('int')
well_rate = (nrow-1) * [ -5. ]
well_rate = random_line( N = nrow-1 , max_value = -10 , length_scale = 0.2)



# well_rate = np.zeros(shape=(nrow))
#random_line( N = nrow , max_value = 10 , length_scale = 0.5)
