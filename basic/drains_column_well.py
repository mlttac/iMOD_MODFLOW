...
def plotfigs(f1,title1,f2,title2,contour1= False, contour2= True, sharedColorbar12 = False):
       ...
       # cbar = fig.colorbar(cntr1, ax=ax1, cmap=cmap)# ticks=classes) # , format='%1i'
       ...
       if contour2== True:
              ...
              ax2.clabel(cset, inline=True, fmt='%1.2f', fontsize=20, colors = 'k')
       ...
       fig.savefig('E4_inputs.svg', format='svg', dpi=1200)
...
def run(z_drain_top = 0, cond_drain = 0.010, n_drains = 11, spacing_drains=1, k_value = None, wells_active = False):
       ...
       x_drain = 10 # start from the left
       # z_drain_top = -1 
       ...
       elevation2[y_drain2,x_drain2] = z_drain_top
       conductance2[y_drain2,x_drain2] = cond_drain
       ...
       if drain_active:
              ...
              #             elevation=elevation2 - dz*spacing_drains,
...
# plt.show()
# plt.close()
# plt.hist(-S.flatten(), density=True)
# plt.show()
# plt.close()
# plt.plot(-S[16,:])
...
if __name__ == "__main__":
       ...
       # DEFINE THE INPUTS:
       outputName = "../data/1well_center_1drain.npz"
       sign = -1 # if I transform to positive values
       num_train = 1000
       num_test = 100
       s = 32
       dy = 2 #(x,y)
       ...
       for realisation, folder in zip(realisations, folders):
              ...
              for iter in range(realisation[0]):
                     ...
                     folder[2].append(sign *  S.reshape(s,s))
